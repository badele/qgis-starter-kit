...
function get_admin_centre() {
    # [out:json]
    # [timeout:120]
    # ;
    # (
    # relation["name"="Montpellier"]["admin_level"=8]["boundary"="administrative"];
    # );
    # out body;
    # >;
    # out skel qt;

    URL="http://overpass-api.de/api/interpreter?data=%5Bout%3Ajson%5D%0A%5Btimeout%3A120%5D%0A%3B%20%20%20%20%20%0A%28%0A%20%20relation%5B%22name%22%3D%22Montpellier%22%5D%5B%22admin_level%22%3D8%5D%5B%22boundary%22%3D%22administrative%22%5D%3B%0A%29%3B%0Aout%3B%0A%3E%3B%0Aout%20skel%3B"
    test ! -f ${TMP_DOWNLOAD}/montpellier_admin_centre_boundary.json && wget -O ${TMP_DOWNLOAD}/montpellier_admin_centre_boundary.json "${URL}"
    test ! -f datas/regions_provinces/languedoc-roussillon/montpellier_admin_centre_boundary.geojson && osmtogeojson ${TMP_DOWNLOAD}/montpellier_admin_centre_boundary.json > datas/regions_provinces/languedoc-roussillon/montpellier_admin_centre_boundary.geojson
...
for REGION in $REGIONS; do
    test ! -f datas/regions_provinces/${REGION}/README -a ! -f ${TMP_DOWNLOAD}/${REGION}.shp.zip && wget -O ${TMP_DOWNLOAD}/${REGION}.shp.zip "http://download.geofabrik.de/europe/france/${REGION}-latest-free.shp.zip"
done
...
# French regions
test ! -f ${TMP_DOWNLOAD}/regions-20180101-shp.zip -a ! -f datas/countries/polygons/regions-20180101.shp && wget -O ${TMP_DOWNLOAD}/regions-20180101-shp.zip https://www.data.gouv.fr/fr/datasets/r/aacf9338-8944-4513-a7b9-4cd7c2db2fa9
...
test ! -f ${TMP_DOWNLOAD}/andorra_spain.json -a ! -f datas/countries/polygons/andorra_spain.geojson && wget -O ${TMP_DOWNLOAD}/andorra_spain.json "${URL}"
...
test ! -f ${TMP_DOWNLOAD}/portugal.json -a ! -f datas/countries/polygons/portugal.geojson && wget -O ${TMP_DOWNLOAD}/portugal.json "${URL}"
...
test ! -f ${TMP_DOWNLOAD}/italia.json -a ! -f datas/countries/polygons/italia.geojson && wget -O ${TMP_DOWNLOAD}/italia.json "${URL}"
...
test ! -f ${TMP_DOWNLOAD}/schweiz.json -a ! -f datas/countries/polygons/schweiz.geojson && wget -O ${TMP_DOWNLOAD}/schweiz.json "${URL}"
